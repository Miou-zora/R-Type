...
bool rtype::utils::AssetsManager::loadTexture(const std::string &name, const std::string &path)
{
    ...
    raylib::Image image = raylib::LoadImageFromMemory(extension.c_str(), data, texture.size());
    m_textures[name] = raylib::LoadTextureFromImage(image);
...
bool rtype::utils::AssetsManager::loadSound(const std::string &name, const std::string &path)
{
    ...
    raylib::Wave wave = raylib::LoadWaveFromMemory(extension.c_str(), data, sound.size());
    m_sounds[name] = raylib::LoadSoundFromWave(wave);
...
bool rtype::utils::AssetsManager::loadFont(const std::string &name, const std::string &path)
{
    ...
    m_fonts[name] = raylib::LoadFontFromMemory(extension.c_str(), data, font.size(), FONT_TTF_DEFAULT_SIZE, NULL, FONT_TTF_DEFAULT_NUMCHARS);
